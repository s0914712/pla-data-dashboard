...
8/28 1600-1700 部務會議        ← 四位數寫法，不用冒號
明天 09:00-10:30 週報會議
下週一 下午2點-4點半 專案檢討
請假 8/29 全天
請假 8/29 1330-1730
...
**時間寫法**有兩種，可以混用（`1600-17:30` 也通）：

| 寫法 | 例 |
|---|---|
| 冒號／「點」 | `14:00-16:00`、`下午2點-4點半` |
| 四位數（軍用） | `1600-1700`、`0830-0945`、`下午0400-0530` |

四位數寫法有邊界防呆：前後必須是空白、字串端或區間分隔符（`- ~ 到 至`），
所以「第1200梯次」「分機0830轉3」「2026年度預算」都不會被讀成時間。
時分不合法（`2500`、`1265`）也不採用。

解析規則：

| 情況 | 處理 |
|---|---|
| 有月日無年份（**建立**行程時）| 選距現在最近且尚未過期的未來日期 |
| 有月日無年份（**查詢**時）| 選絕對距離最近的那一年 —— 9/1 查 `8/28` 是四天前，不是明年 |
...
**查詢請看下面的「查詢」一節** —— 記事沒有自己的查詢指令，
`查記事` 與 `查行程` 是同一個入口，結果是行程、請假、記事的合併檢視。

**日期推論**：建立記事時會用行程解析器的同一套日期邏輯掃描內容
（`inferDate()`），抓到就寫入 `target_date`，抓不到就是 `null`。
查詢據此歸戶：優先用 `target_date`，沒有才退回建立當天（台北時區）。
掃描有防呆——數字前後緊鄰其他數字時不採用，
...
**`查記事` 與 `查行程` 完全同義**，解析出來的條件與回覆的內容一模一樣。
兩者本來就常在講同一件事，分成兩條指令只會讓人記不住該打哪個。

```
查行程 ／ 查記事 ／ 行程 ／ 記事      ← 不加參數就是今天
明天 ／ 明天行程 ／ 查 8/28          ← 沒有指令詞的裸日期也行
查行程 8/28-8/31                    ← 日期區間（~ 到 至 都可以）
查行程 本週 ／ 本月 ／ 上週 ／ 最近7天
查行程 本週請假 ／ 誰請假            ← 只看請假
查行程 明天會議                      ← 只看會議
...
查詢參數由 `parseQueryArg()` 解析成一個 `QueryTarget`
（`from` / `to` / `filter` / `keyword`），選單按鈕送的 `act=q&s=…`
走的是同一支解析器 —— 按鈕與打字只有一套語法。
...
| 單一日期 | 那一天 |
| 日期區間、期間詞、滾動區間 | 該區間 |
| 只有關鍵字 | 日期交給呼叫端，預設往前 90 天、往後 180 天 |

**兩種排版**：

- **單日且沒有關鍵字** → `renderDayView()`，維持【行程】【請假】【記事】
  三區，一眼看得出當天全貌。
- **區間或帶關鍵字** → `renderRangeView()`，依日分段的密集排版，
  每行掛符號（請假 🏖、記事 📝、行程無），是拿來掃的。超過 70 行會截斷並提示。

**跨日的全天事件會攤平到涵蓋的每一天**。請假 8/29-8/31 若只掛在起日，
查「本週誰不在」就會漏掉後面兩天。`DayEvent` 因此帶 `date` 與 `endDate`
（Google 的 `end.date` 是排除式，讀回來時先減一天）。

資料來源：

- **行程／請假**：Google Calendar 的 `events.list`（`singleEvents=true`），
  所以在手機日曆 App 裡直接新增、不經過小助手的事件也會列出。
  關鍵字走 Google 的 `q` 參數（比對標題、說明、地點）。
- **記事**：Supabase 的 `notes_in_range()`。

Google 讀取失敗時仍會列出記事，並在訊息尾端附上錯誤原因——兩邊分開容錯。

**與建立語句的界線**（合併後仍然成立，有測試守著）：沒有指令詞時，
剝掉前後綴後必須「剛好只剩一段日期」才算查詢。所以
`明天 09:00-10:30 週報會議`、`請假 8/29-8/31`、`本週三 1400-1600 部務會議`
都仍然是建立，不會被查詢吃掉。
...
| `notes` | 記事（軟刪除）。`target_date` 為推論出的歸屬日期，可為 null。查詢一律走 `notes_in_range()`（區間＋關鍵字，關鍵字的 `%` `_` `\` 會先跳脫）；舊的 `notes_for_date()` 仍在 DB 裡但程式已不再呼叫 |
...
| 記事沒歸到預期的日期 | 內容裡的日期沒被辨識，或被防呆擋掉。用「查行程 最近7天」確認，必要時改寫成「記事 8/28 要交報告」|
