...
查記事 報告                  ← 關鍵字
查記事 8/28                  ← 某一天
查記事 8/28-8/31             ← 日期區間（~ 到 至 都可以）
查記事 本週                  ← 期間詞
查記事 本週 報告             ← 區間＋關鍵字
刪記事 3
```

**日期區間查詢**（`notes_in_range()`）支援四種寫法，可與關鍵字混用：

| 形態 | 例 |
|---|---|
| 單一日期 | `8/28`、`明天`、`下週一` |
| 日期區間 | `8/28-8/31`、`8/28~8/31`、`8/28 到 8/31` |
| 期間詞 | `本週`、`上週`、`下週`、`本月`、`上個月`、`下個月` |
| 滾動區間 | `最近7天`、`近3天`、`最近一週`、`最近一個月` |

「本週」以**週一為首日**（與行程解析的「本週三」定義一致）。
迄日早於起日時：沒寫年份當跨年（`12/28-1/3`），寫了年份當作寫反了自動對調。
歸戶日期用 `target_date`，沒有才退回建立當天（台北時區）。單次上限 50 筆。

不想打字的話：選單 →「**選日期區間查記事**」→ 選起日 → 選迄日。
起日直接編進第二張卡的 postback（`act=nt_to&s=YYYY-MM-DD`），不占用草稿狀態；
兩個日期選反了也會自動對調。選單另有「**本週記事**」「**本月記事**」兩顆快捷鍵，
它們送的是 `act=notes&q=本週`，走的就是上面同一條文字語法。
...
請假與行程的引導式入口、**本週／本月記事**、**選日期區間查記事**、
**最近 10 筆**、**使用說明**。看不懂的訊息也回這張卡，而不是丟一大串文字。
...
| `notes` | 記事（軟刪除）。`target_date` 為推論出的歸屬日期，可為 null。查詢用 `notes_for_date()`（單日）與 `notes_in_range()`（區間＋關鍵字，關鍵字的 `%` `_` `\` 會先跳脫）|
